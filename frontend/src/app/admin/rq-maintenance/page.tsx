...
export default function AdminRqMaintenancePage() {
  ...
  return (
    <div className="p-6 md:p-8 space-y-6 max-w-5xl w-full mx-auto animate-fadeIn">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-black text-slate-900">กล่องแจ้งซ่อม</h1>
        <p className="text-slate-500 text-sm font-semibold">มอบหมายงาน ตรวจสอบภาพถ่ายจากผู้เช่า และแก้ไขรายการแจ้งซ่อมห้องพัก</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
        ...
        <div className="relative md:col-span-6">
          ...
          <input
            type="text"
            placeholder="ค้นหาด้วยหมายเลขห้องหรือประเภทงาน..."
            value={searchTerm}
        ...
        <div className="md:col-span-3 flex rounded-2xl border border-[#FFC193]/50 bg-white p-1 shadow-sm">
          {(['All', 'Open', 'In Progress', 'Resolved'] as const).map(tab => (
            ...
            >
              {tab === 'All' ? 'ทั้งหมด' : tab === 'Open' ? 'เปิดใหม่' : tab === 'In Progress' ? 'กำลังทำ' : 'เสร็จแล้ว'}
            </button>
          ))}
        </div>
        ...
        <div className="md:col-span-3 flex rounded-2xl border border-[#FFC193]/50 bg-white p-1 shadow-sm">
          {(['All', 'Low', 'Medium', 'High'] as const).map(tab => (
            ...
            >
              {tab === 'All' ? 'ทั้งหมด' : tab === 'Low' ? 'ต่ำ' : tab === 'Medium' ? 'ปานกลาง' : 'สูง'}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase border ${
                    ...
                    ระดับ: {tkt.priority === 'High' ? 'สูง' : tkt.priority === 'Medium' ? 'ปานกลาง' : 'ต่ำ'}
                  </span>
                  ...
                  <span className={`text-[10px] px-2.5 py-0.5 rounded-full font-black uppercase ${
                    ...
                    {tkt.status === 'In Progress' ? 'กำลังดำเนินการ' : tkt.status === 'Open' ? 'เปิดใหม่' : 'แก้ไขแล้ว'}
                  </span>
                </div>
        ...
        {filteredTickets.length === 0 && (
          <div className="py-16 text-center bg-white rounded-2xl border border-[#FFC193]/30">
            ...
            <p className="font-bold text-slate-400 text-sm">ไม่พบรายการแจ้งซ่อม</p>
          </div>
        )}
      </div>
