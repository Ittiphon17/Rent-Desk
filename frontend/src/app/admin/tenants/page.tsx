...
export default function AdminTenantsPage() {
  ...
  const handleEvict = (id: string, unit: string, name: string) => {
    if (confirm(`Are you sure you want to end the lease for ${name} in unit ${unit}? This will evict the tenant and set the room status back to Vacant.`)) {
    ...
    <div className="p-6 md:p-8 space-y-6 max-w-5xl w-full mx-auto animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm font-semibold">Track currently active leases, register new residents, and manage occupancy history.</p>
        </div>
      ...
      <div className="relative">
        ...
        <input
          type="text"
          placeholder="Search by Room Unit or Tenant Name..."
          value={searchTerm}
                ...
                filteredTenants.map(tenant => (
                  <tr key={tenant.id} className="hover:bg-[#FFEDCE]/10 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      ...
                      <span className={`inline-flex items-center gap-0.5 text-[9px] font-black uppercase px-2 py-0.5 mt-1.5 rounded-full ${
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fadeIn">
          <div className="w-full max-w-lg overflow-hidden rounded-2xl border border-[#FFC193]/50 bg-white shadow-xl">
            <div className="flex items-center justify-between border-b border-slate-150 p-5 bg-[#FFEDCE]/30">
              <h3 className="text-base font-bold text-slate-900">Edit Resident Information</h3>
            ...
            <form onSubmit={handleEditSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase">Room Unit</label>
                  <select
                    ...
                    className="mt-1 block w-full rounded-xl border border-[#FFC193]/60 bg-slate-50/50 p-2.5 text-sm text-slate-800 outline-none focus:border-[#FF3737]"
                  >
                    <option value={editingTenant.unit}>{editingTenant.unit} (Current)</option>
